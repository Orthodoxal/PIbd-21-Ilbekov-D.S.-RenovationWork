...
namespace RenovationWorkBusinessLogic.OfficePackage.Implements
{
    public class SaveToExcel : AbstractSaveToExcel
    {
        ...
        private static void CreateStyles(WorkbookPart workbookpart)
        {
            ...
            var differentialFormats = 
                new DocumentFormat.OpenXml.Office2013.Excel.
        ...
        protected override void InsertCellInWorksheet(ExcelCellParameters excelParams)
        {
            ...
            if (sheetData.Elements<Row>().
                Where(r => r.RowIndex == excelParams.RowIndex).Any())
            {
                row = sheetData.Elements<Row>().
                    Where(r => r.RowIndex == excelParams.RowIndex).First();
            }
            else
            {
            ...
            if (row.Elements<Cell>().
                Where(c => c.CellReference.Value == excelParams.CellReference).Any())
            {
                cell = row.Elements<Cell>().
                    Where(c => c.CellReference.Value == excelParams.CellReference).First();
            }
            else
            {
            ...
            _shareStringPart.SharedStringTable.
                AppendChild(new SharedStringItem(new Text(excelParams.Text)));
            _shareStringPart.SharedStringTable.Save();
            cell.CellValue = 
                new CellValue((_shareStringPart.SharedStringTable.Elements<SharedStringItem>().Count() - 1)
                .ToString());
